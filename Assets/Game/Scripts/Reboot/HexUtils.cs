...
public struct Layout
{
    ...
    {
        Orientation M = Orientation;
        float angle = 2.0f * Mathf.PI * (M.start_angle - (float)corner) / 6f;
        return new Vector2(Size.x * Mathf.Cos(angle), Size.y * Mathf.Sin(angle));
    }

    public List<Vector2> PolygonCorners(Hex h, float size = 1.0f)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            corners.Add(new Vector2(center.x + (offset.x * size), center.y + (offset.y * size)));
        }
        return corners;
    }
}
...
struct Tests
{
    ...
    {
        Hex h = new Hex(3, 4, -7);
        Layout flat = new Layout(Layout.FLAT, new Vector2(10, 15), new Vector2(35, 71));
        Tests.EqualHex("layout", h, flat.PixelToHex(flat.HexToPixel(h)).HexRound());
        Layout pointy = new Layout(Layout.POINTY, new Vector2(10, 15), new Vector2(35, 71));
